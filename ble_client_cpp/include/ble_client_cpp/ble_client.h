...
class BleClient
{
    public:
        ...
        void gattWriteCmd(const uint16_t _handle,
                const uint8_t *_data,
                const int _size);
        void gattListenToNotification(
                void (*)(const uint16_t, const int,const uint8_t*));
    ...
    private:
        ...
        static std::vector<void(*)(const uint16_t, const int,const uint8_t*)> notificationCbFcts;

        static void * startGLoop(void *);

        static gboolean channel_watcher(GIOChannel *chann, GIOCondition cond, gpointer user_data);
